...
def move_type(board, x, y, last_x, last_y, selection_flag):  # check whether a move is valid, etc.
    if not selection_flag:  # now no grid has been selected
        ...
        if get_grid_id(x, y) in peices_position(board):
            return SELECT_A_PIECE
        if chess.Move(get_grid_id(last_x, last_y), get_grid_id(x, y)) in board.legal_moves or chess.Move(
                get_grid_id(last_x, last_y), get_grid_id(x, y), promotion=chess.QUEEN) in board.legal_moves:
            return VALID_MOVE
        return SELECT_INVALID_GRID
